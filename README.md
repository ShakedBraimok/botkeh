...
- **[Option 1: GitHub Actions (Automated)](#option-1-github-actions-automated-deployment)** - Recommended for teams, automatic deployment on merge to main
- **[Option 2: Manual Deployment](#option-2-manual-deployment)** - Deploy locally using Makefile commands
...
### Step 1: Configure AWS IAM OIDC

GitHub Actions uses AWS IAM OIDC to authenticate without storing credentials. Here's how to set it up:

**A. Create IAM OIDC Provider**
...
export GITHUB_REPO="YOUR_USERNAME/YOUR_REPO"  # e.g., myorg/template-slackbot
...
# Create the role
aws iam create-role \
  --role-name GitHubActions-SlackBot-Role \
...
aws iam get-role --role-name GitHubActions-SlackBot-Role --query Role.Arn --output text
```

**C. Add Secret to GitHub Repository**

1. Go to your GitHub repository → **Settings** → **Secrets and variables** → **Actions**
2. Click **New repository secret**
3. Name: `AWS_ROLE_ARN`
4. Value: `arn:aws:iam::YOUR_ACCOUNT_ID:role/GitHubActions-SlackBot-Role`
5. Click **Add secret**
...
3. **Save Bot Token** (starts with `xoxb-`)
4. Navigate to **"Basic Information"** → **"App Credentials"**
5. **Save Signing Secret**

### Step 3: Configure Slack Credentials in AWS
...
aws secretsmanager create-secret \
  ...
  --secret-string '{"slack_bot_token":"YOUR_SLACK_BOT_TOKEN_HERE","slack_signing_secret":"YOUR-SECRET"}' \
  --region us-east-1
```
...
3. Copy the API Gateway URL from the workflow summary

### Step 5: Update Slack App URLs

1. Go to Slack app → **"Slash Commands"**
2. Create command: `/senora` → Set Request URL to API Gateway URL
3. Go to **"Interactivity & Shortcuts"**
4. Set Request URL to same API Gateway URL

### Step 6: Test

Open Slack and type `/senora` - you should see the bot respond!

---

## Option 2: Manual Deployment

**Best for:** Local development, testing, single deployments
...
4. **Copy Bot User OAuth Token** (starts with `xoxb-`)
   - Example: `YOUR_SLACK_BOT_TOKEN_HERE`
...
# This creates:
# bot/src/slack_bot/actions/deploy-api/
#   ├── modal.json
#   └── buildspec.yaml
```
...
Edit `bot/src/slack_bot/actions/deploy-api/modal.json`:
...
Edit `bot/src/slack_bot/actions/deploy-api/buildspec.yaml`:
